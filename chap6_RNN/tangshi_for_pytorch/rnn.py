...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim,device):
        ...
        # self.tanh = nn.Tanh()
    def forward(self,sentence,is_test = False,hidden = None):
        ...
        # ???
        if hidden is None:
            h_0 = Variable(sentence.data.new(2, 1, self.lstm_dim).fill_(0).float()).to(self.device)
            c_0 = Variable(sentence.data.new(2, 1, self.lstm_dim).fill_(0).float()).to(self.device)
        else:
            h_0, c_0 = hidden
        ...
        output, hn = self.rnn_lstm(batch_input, (h_0, c_0))
